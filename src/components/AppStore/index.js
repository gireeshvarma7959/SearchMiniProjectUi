...
class AppStore extends Component {
  ...
  getFilteredApps = searchResults => {
    ...
    const filteredApps = searchResults.filter(
      eachItem => eachItem.category === isActiveTab,
    )
  ...
  updateApps = tabId => {
    this.setState({isActiveTab: tabId})
  }

  getSearchResult = () => {
    const {searchInput} = this.state
    const searchResults = appsList.filter(everyItem =>
  ...
  render() {
    const {isActiveTab, searchInput} = this.state
    const searchResults = this.getSearchResult()
    const filteredAppValues = this.getFilteredApps(searchResults)
        ...
        <ul className="tab-lists">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={eachTab.tabId === isActiveTab}
            />
          ))}
        </ul>
        <div className="all-apps-container">
          <ul className="apps-lists">
            {filteredAppValues.map(eachApp => (
